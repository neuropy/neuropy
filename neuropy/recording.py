...
class BaseRecording(object):
    ...
    def collectcchs(self, nids, trange, bins, shift=False, normalize=False):
        """Collect cross-correlogram histograms for all pairs in nids. trange and bins are in
        us. If shift, then shift correct the CCH by subtracting the CCH generated by shifting
        one spike train in each pair by some random amount. If normalize, weight each pair
        equally."""
        n = self.n
        nn = len(nids)
        hists = []
        if shift:
            nshifts = int(shift)
            minshift = max(abs(trange))
            maxshift = 2 * minshift
            shiftslope = maxshift - minshift
        for nii0 in range(nn):
            for nii1 in range(nii0+1, nn):
                ...
                if shift:
                    # randomize amplitude of shifts:
                    tshifts = intround(minshift + shiftslope*np.random.random(nshifts))
