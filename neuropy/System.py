...
class System(object):
    ...
    def __init__(self, name=DEFAULTSYSTEMNAME, parent=_model):
        ...
        self.r = dictattr() # store Runs in a dictionary with attrib access
    def tree(self):
    ...
    def load(self):
        ...
        runNames = [ dirname for dirname in os.listdir(self.path)
                     if os.path.isdir(os.path.join(self.path, dirname))
                     and dirname[0:2].isdigit()
                     and dirname.count(' - ') == 1 ] # 1st 2 chars in dirname must be digits, must contain exactly 1 occurrence of ' - '
