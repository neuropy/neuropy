...
class BaseExperiment(object):
    ...
    def get_sweeptable(self):
        try:
            return self._sweeptable
        except AttributeError:
            pass # hasn't been built yet
        if self.textheader == '': # no sweeptable to build
            raise AttributeError("can't build sweeptable with empty textheader")
        ...
        if self.__version__ >= 0.16: # after major refactoring of dimstim
            ...
            if m:
                ...
                ## TODO: not sure why sweeptable is being bound to m, historical reasons?
                ...
                # pass varlist by reference, dim indices end up being modified:
                self._sweeptable = self.buildptc15SweepTable(self.oldparams.varlist, varvals,
    ...
    sweeptable = property(get_sweeptable) # generate only on use
    st = property(get_sweeptable) # synonym, used a lot by experiment subclasses
